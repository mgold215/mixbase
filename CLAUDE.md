...
- **Apple Developer Program is ACTIVE (paid; team `AP8UC39D4D`); the app ships continuously to TestFlight and has an App Store Connect record + API key.** **Version 1.0 is LIVE on the App Store since 2026-09-15** (https://apps.apple.com/app/mixbase/id6763808906 — submission 902f9887, build 202609081333, approved after 7 submissions: 3.1.1 paid-content findings, 1.2 UGC, 5.1.1 deletion, 2.1 info requests). What finally passed: native in-app Terms/Privacy/Support (no links to the website), UGC report/block backed by real tables, and NO paid plans anywhere. Never suggest joining the program or forming anything.
- **After every App Store release, bump `MARKETING_VERSION` in `ios/mixBase.xcodeproj/project.pbxproj` (all four build configurations, app + widget) before the next iOS merge.** Apple closes the released train: from 2026-09-15 to 09-16 every TestFlight upload failed with "The train version '1.0.0' is closed for new build submissions" until 1.0.0 → 1.0.1 (PR #163). The macOS lane kept working only because the Mac has no released version yet.
...
- **App Store listing lane** (`.github/workflows/app-store-listing.yml`, push-driven TEMP branch `asc7-listing`, orders in `.github/asc-maintenance/listing-request.json`, copy in `listing-copy.json`): builds the app for an iPhone 17 Pro Max simulator on a macOS runner, signs in as the App Review demo account (login read from the version's review detail via the ASC API, so no repo secret), walks the app with the XCUITest tour in `ios/screenshots/` (XcodeGen bundle, no host app), commits the 1320x2868 PNGs to `ios/metadata/screenshots/` on the branch, then `asc-listing.mjs` creates/edits the version (copy, subtitle, copyright, review details), replaces the iPhone screenshot sets (6.5" derived by resize), attaches the newest processed build and submits only with `submit: true`. `"capture": false` skips the simulator and publishes from the committed PNGs. Promotional text is the only listing field editable on a live version; subtitle, description, keywords and screenshots need a new version. The demo account (review@mixbase.app) was seeded 2026-09-16 with the developer's three released singles (3–4 mixes each, feedback, releases, library, a collection, real cover art from the released catalog) so every screen has content — keep it that way; the seed job refreshes only the artwork.
- App Store: LIVE (1.0, 2026-09-15; see iOS section). **Territory availability is API-managed**: the app record shipped with NO availability at all (every read of `GET /v1/apps/{id}/appAvailabilityV2` was 404, so the store said "not available in your country or region" even though 1.0 was Ready for Distribution) — created 2026-09-15 via `POST /v2/appAvailabilities` (all 175 territories, `availableInNewTerritories: true`, run 27 on `asc6-submit`). If that store message ever recurs, read that endpoint first. Future version submissions/notes updates run through the push-driven TEMP branches of `.github/workflows/asc-resubmit.yml` (`asc5-inspect` = read-only state dump, `asc6-submit` = attach newest post-fix build + notes + submit); workflow_dispatch is 403 for remote sessions, and Apple's Resolution Center has NO API — replies to App Review must be typed in App Store Connect. If App Store metadata references privacy@/support@/dmca@/legal@/review@ mixbase.app, verify those aliases exist — don't frame them as pre-submission blockers.
