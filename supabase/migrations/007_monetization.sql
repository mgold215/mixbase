...
-- Unique partial indexes so the same Stripe/Apple ID can't appear on two profiles,
...
-- Index to make per-user monthly lookups fast
CREATE INDEX IF NOT EXISTS mb_usage_user_month_idx
  ON public.mb_usage (user_id, month);

-- Enable Row Level Security -- users can only see and modify their own rows
ALTER TABLE public.mb_usage ENABLE ROW LEVEL SECURITY;

-- Issue 1 fix: DROP before CREATE so re-running this migration is idempotent
...
-- Issue 3 fix: UPDATE policy now includes WITH CHECK so the new row's
-- user_id is also validated, not just the existing row.
...
-- ============================================================
-- 3. Helper functions -- increment usage counters atomically
--    security definer: runs with the privileges of the owner,
--    not the calling user, so it can bypass RLS for the upsert.
--    search_path = public prevents schema-injection attacks.
-- ============================================================

-- Increment artwork_generations by 1 for a given user + month.
...
-- Increment video_generations by 1 for a given user + month.
-- Creates the row if it does not exist yet (upsert).
...
-- Issue 2 fix: Prevent anon/authenticated clients from calling these functions
-- directly with an arbitrary p_user_id (which would let any user inflate
-- another user's usage counter). Only the service-role key (server-side) can
-- call them. The SECURITY DEFINER bypass is intentional for the server only.
