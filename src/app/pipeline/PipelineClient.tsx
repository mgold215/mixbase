...
function MetaInput({ release, field, label, placeholder, onSave }: {
  ...
  label: string
  placeholder?: string
  // Returns a promise in practice (updateField), but callers may ignore it.
  onSave: (releaseId: string, field: string, value: string | null) => void | Promise<unknown>
}) {
  const value = (release[field] as string | null) ?? ''
  const [draft, setDraft] = useState(value)
  const [editing, setEditing] = useState(false)
  // Bumped each time a save settles. It joins the adoption key below so that a
  // save whose canonical result is IDENTICAL to the pre-save value still
  // re-syncs the box. Clearing Language PATCHes null, the server coerces it
  // straight back to 'English' (coerceReleaseNulls), so `value` never changes —
  // and keyed on `value` alone the adopt branch never fired, leaving the field
  // visibly EMPTY while the database still said English.
  ...
  const adoptKey = `${saveTick}\u0000${value}`
  const [prevKey, setPrevKey] = useState(adoptKey)
  if (prevKey !== adoptKey) {
    setPrevKey(adoptKey)
    ...
    <div>
      ...
      <input
        ...
        onBlur={e => {
          ...
          if (v !== value) {
            // Re-sync once the save settles, whatever the server decided. This
            // also reverts the box to the last known-good value when the save
            // FAILS (updateField resolves false without touching state), rather
            // than stranding text that was never persisted.
            void Promise.resolve(onSave(release.id, field, v || null))
              .then(() => setSaveTick(t => t + 1))
              .catch(() => setSaveTick(t => t + 1))
          }
          else setDraft(value)
        }}
